...
dashboard_blueprint = Blueprint('dashboard_blueprint',_name_)
...
def todayVisitors():
    ...
    sql = text('SELECT COUNT(id) AS today_visitors FROM visitor_log WHERE date = "'+currentDate+'"')
...
def overallVisitors():
    sql = text("SELECT COUNT(id) AS overall_visitors FROM visitor_log")
    result = db.engine.execute(sql)
    rawdata = result.fetchall()
    jsondata = jsonify([dict(row) for row in rawdata])
    return jsondata
...
def maleVisitors():
    ...
    sql = text('SELECT COUNT(id) AS male_visitors FROM visitor_log WHERE date = "'+currentDate+'" AND gender = 1')
    result = db.engine.execute(sql)
    rawdata = result.fetchall()
    jsondata = jsonify([dict(row) for row in rawdata])
    return jsondata
...
def femaleVisitors():
    ...
    sql = text('SELECT COUNT(id) AS female_visitors FROM visitor_log WHERE date = "'+currentDate+'" AND gender = 2')
    result = db.engine.execute(sql)
    rawdata = result.fetchall()
    jsondata = jsonify([dict(row) for row in rawdata])
    return jsondata
...
def ageGroupClassfication():
    ...
    for a in range(1,6):
        ...
        for g in range(1,3):
            ...
            # Today's Visitors
            sqlDvisitors = text('SELECT COUNT(id) AS age_today_visitors FROM visitor_log WHERE date = "'+currentDate+'" AND gender = "'+str(g)+'" AND age_group = "'+str(a)+'"')
            ...
            # Overall Visitors
            sqlOverallVisitors = text('SELECT COUNT(id) AS age_overall_visitors FROM visitor_log WHERE gender = "'+str(g)+'" AND age_group = "'+str(a)+'"')
...
def bargraph():
    ...
    for m in range(1,13):
        ...
        sql = text('SELECT COUNT(id) AS total_month_visitors FROM visitor_log WHERE Month(date) = "'+str(m)+'"')
        result = db.engine.execute(sql)
        rawdata = result.fetchall()
